...
function loadParticle(url, type, particleHeight) {
  ...
  fetch(url)
    ...
    .then((blob) => {
      ...
      particleObj2 = new Particle3D(viewer, {
        input: blob,
      ...
      particleObj2.init().then((res) => {
        particleObj2.show()
        addClickHandler(particleObj2.data)
        // addGUI()
      ...
      function updateOptions() {
        particleObj2.optionsChange(systemOptions)
      }
      // setTimeout(() => {
      //   updateOptions()
      // }, 100)
  ...
  // particleObj2.optionsChange(systemOptions) // 更新粒子系统配置

  // particleObj2.hide() // 停止粒子系统
  // particleObj2.remove() // 移除粒子系统
}
//点击获取粒子值
var ClickHandler
