...
function loadParticle(url, type, particleHeight) {
  ...
  fetch(url)
    ...
    .then((blob) => {
      ...
      particleObj5 = new Particle3D(viewer, {
        input: blob,
      ...
      particleObj5.init().then((res) => {
        particleObj5.show()
        addClickHandler(particleObj5.data)
        // addGUI()
      ...
      function updateOptions() {
        particleObj5.optionsChange(systemOptions)
      }
      // setTimeout(() => {
      //   updateOptions()
      // }, 100)
  ...
  // particleObj5.optionsChange(systemOptions) // 更新粒子系统配置

  // particleObj5.hide() // 停止粒子系统
  // particleObj5.remove() // 移除粒子系统
}
//点击获取粒子值
var ClickHandler
