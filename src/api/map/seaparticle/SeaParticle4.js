...
function loadParticle(url, type, particleHeight) {
  ...
  fetch(url)
    ...
    .then((blob) => {
      ...
      particleObj4 = new Particle3D(viewer, {
        input: blob,
      ...
      particleObj4.init().then((res) => {
        particleObj4.show()
        addClickHandler(particleObj4.data)
        // addGUI()
      ...
      function updateOptions() {
        particleObj4.optionsChange(systemOptions)
      }
      // setTimeout(() => {
      //   updateOptions()
      // }, 100)
  ...
  // particleObj4.optionsChange(systemOptions) // 更新粒子系统配置

  // particleObj4.hide() // 停止粒子系统
  // particleObj4.remove() // 移除粒子系统
}
//点击获取粒子值
var ClickHandler
