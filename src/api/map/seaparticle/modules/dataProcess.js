...
import netcdfjs from './netcdfjs.min'
  ...
  var loadNetCDF = function (file, { fields, valueRange, offset }) {
    return new Promise(function (resolve, reject) {
      ...
      reader.onload = function () {
        ...
        // console.log('dem数据_____')
        // console.log(data.dem)
        // // 定义比例系数
        // const scaleFactor = -20000.0 // 这里可以替换为您的实际比例系数

        // // 按比例调整 data.dem.array 的值
        // if (data.dem && data.dem.array) {
        //   for (let i = 0; i < data.dem.array.length; i++) {
        //     data.dem.array[i] *= scaleFactor // 将每个元素按比例乘以 scaleFactor
        //   }

        //   // 更新 min 和 max 值
        //   data.dem.min = Math.min(...data.dem.array)
        //   data.dem.max = Math.max(...data.dem.array)
        // }
        resolve(data)
      }
    })
