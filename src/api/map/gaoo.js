...
//     });
// };
function loadaotao1(sort_url, tri_url, type_1, color_1, id_1, high,point_url,op) {
  Promise.all([
    fetch(sort_url).then(response => response.json()),
    fetch(tri_url).then(response => response.json()),
    fetch(point_url).then(response => response.json())
  ])
    .then(([coordsData, indicesData, pointData]) => {
      ...
      pointData.features.forEach((feature2) => {
        ...
        const labelPosition = Cesium.Cartesian3.fromDegrees(coords[0], coords[1], 0);
      //   const labelPositionv = Cesium.Cartesian3.fromDegrees(coords[0], coords[1], 10000);
      //   viewer.camera.flyTo({
      //     destination: labelPositionv,
      // }); // 转换为Cartesian3
        ...
        label2.push({label:labelCollection,id:id_1});
...
function clearyuanyan1(id) {
  ...
  label2 = label2.filter(item => {
    ...
    window.viewer.scene.primitives.remove(item.label);
  return false}
  return true
    // 保留其他对象
  });
