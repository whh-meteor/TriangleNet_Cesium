...
export class CesiumPopupContextmenuUtil {
    ...
    if (this.option.container) {
      this.option.container.addEventListener(
        ...
        function (e) {
          ...
          const menu = self.setPosition(
            clientX - self.option.container.getBoundingClientRect().left,
            clientY - self.option.container.getBoundingClientRect().top
          )
          if (
            ((_a = self.option) === null || _a === void 0
              ? void 0
              : _a.onRClick) &&
            menu
          ) {
  ...
  getMenu(container) {
    ...
    // const actionNames = this.option.actionNames
    //   ? this.option.actionNames
    //   : [
    //       CesiumPopupContextmenuActionNames.move,
    //       CesiumPopupContextmenuActionNames.editAttr,
    //       CesiumPopupContextmenuActionNames.remove
    //     ]
    // const obj = menuObj ? menuObj : {}
    // obj[CesiumPopupContextmenuActionNames.move] =
    //   '<li onclick="earthpopupmove()">编辑位置</li>'
    // obj[
    //   CesiumPopupContextmenuActionNames.remove
    // ] = `<li onclick="earthpopupremove()">删除</li>`
    // obj[CesiumPopupContextmenuActionNames.editAttr] =
    //   '<li onclick="earthpopupedit()">编辑样式</li>'
    // obj[CesiumPopupContextmenuActionNames.stopMove] =
    //   '<li onclick="earthpopupstopMove()">停止编辑位置</li>'
    // let html = ''
    // for (let i in actionNames) {
    //   if (!isNaN(Number(i))) {
    //     const name = actionNames[i]
    //     if (name) html += obj[name]
    //   }
    // }
    // el.innerHTML = html
    // if (container) {
  ...
  remove() {
    ...
    if (this.option.container) {
      ...
      _window.earthpopupedit = () => {}
      _window.earthpopupstopMove = () => {}
      ;(_a = document.getElementById(earthMenuId)) === null || _a === void 0
        ? void 0
        : _a.remove()
    }
  }
